...
<html lang="es">
  <head>
    ...
    <script type="module">
      ...
      if (codes) {
        codes.forEach((code) => (code.innerHTML = highlight(cleanIndent(code.innerText))));
      }
      const languages = [
        ...
        selector.onchange = async (event) => {
          ...
          try {
            ...
            if ("Translator" in self) {
              ...
              const traduction = await traductor.translate(
              ...
              h1Element.innerText = await traductor.translate(
                h1Element.innerText
              );
              h2Elements.forEach(
                async (el) =>
                  (el.innerText = await traductor.translate(el.innerText))
              );
              h3Elements.forEach(
                async (el) =>
                  (el.innerText = await traductor.translate(el.innerText))
      ...
      <div class="aside-main-container">
        ...
        <main>
          <div id="breadcrumbs">
            ...
            <a href="/">Inicio</a> /
            <a href="https://solid-geolocation.vercel.app/docs">Docs</a>
          </div>
          <section id="introduccion">
            <h2>Introducción</h2>
            <div style="display: flex; margin-block: 8px;">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
          ...
          <section id="limites">
            ...
            <p>
              ...
              <strong>60 peticiones por hora</strong>. Si ha sobrepasado el límite
              dentro de la hora, deberá esperar el tiempo que sea necesario para
          ...
          <section id="uso-coordenadas">
            ...
            <p>
              Para poder hacer uso de éste <strong>end-point</strong>, se deberá
              proveer los parmámetros de <code>latitud</code> y
              <code>longitud</code>. Desde el front-end podemos hacer uso de la
              api del navegador de geolocalización y realizar el fecth a dicho
              end-point, dejo éste ejemplo de como podría implementarse:
            </p>
            <pre>
    ...
    "closestAirport": {
        ...
        "distance": "17.116mts"
          ...
          <section id="codigos-estado" class="codigos-estado">
            ...
            <ul>
              <li><code style="color: #4caf50;">200 OK</code> – Petición exitosa.</li>
              <li>
                <code style="color: #ff9800;">400 Bad Request</code> – Parámetros inválidos o faltantes.
              </li>
              <li>
                <code style="color: #f44336;">429 Too Many Requests</code> – Se ha superado el límite de
                peticiones por hora.
              </li>
              <li>
                <code style="color: #9c27b0;">500 Internal Server Error</code> – Error en el servidor.
              </li>
            </ul>
          </section>
        </main>
